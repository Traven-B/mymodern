...
module MyModern
  ...
  def self.setup(@@http_client : ClientUnion = HTTP::Client, @@sequential : Bool = false)
    self
  end
  ...
  def self.concurrent_network_part
    if !@@sequential
      ...
      the_channels.map do |channel_module_pair|
      ...
      # map block returns array of {{checked_out: String, on_hold: String}, Springfield:Module | Shelbyville:Module}
    else
      # Sequential libraries
      MODULE_NAMES.map do |a_module|
        the_channel = PagesChannel.new
        spawn fetch_pair(the_channel, a_module.lib_data)
        fetched_pair_of_pages = the_channel.receive # Wait for THIS library before next
  ...
  def self.fetch_pair(pair_of_webpage_results_channel, data_param)
    ...
    # Duplicated spawn/receive blocks - cleanest
    if !@@sequential
      # Concurrent gets
      # Spawn
